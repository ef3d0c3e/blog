...
lm_small <- lm(time ~ byte_size, data = data_long)
...
line_small <- data.frame(
  byte_size = seq(min(data_long$byte_size),
                  max(data_long$byte_size), length.out = 50)
)
...
p1 <- ggplot(data_long, aes(x = log2_time, y = byte_size)) +
  ...
  geom_violin(
			  alpha = 0.6, color = "gray30", scale = "width"
			  ) +
  geom_point(
    data = stats_data,
  ...
  labs(
    ...
    subtitle = sprintf(
      "Linear regression: slope=%.4f, R²=%.4f, p=%.2e",
